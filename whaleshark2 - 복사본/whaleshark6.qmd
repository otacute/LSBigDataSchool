...
=======
            href: https://github.com/otacute/whaleshark
>>>>>>> 99027217c74d833cee8c49bc925ad1cb819a6f0c
        css: styles.css
editor: 
  markdown: 
    wrap: 72
...
## 통근수단 + 인종 데이터 시각화 
### Column {width=40%}
::: {.card title="**Ames 시민들의 2022년 주요 통근 수단**" style="height: 600px;"}
```{python}
import pandas as pd
...
::: {.card title="**Ames시 연도별(2013-2022) 인종 비율**" style="height: 600px;"}
```{python}
#| output: false
import pandas as pd
import plotly.express as px
...
## Occupations 시각화
### Column {width=40%}
::: {.card title="**Ames시 남성 직종 별 종사율(2013-2022년)**" style="height: 270px;"}
```{python}
#| output: false
import pandas as pd
import plotly.express as px
...
fig.update_traces(
    ...
    textfont=dict(size=18),  # Subgroup 텍스트 크기 크게 조정
)
...
### Column {width=60%}
::: {.card title="**Ames시 직종별 중위값 평균 소득(2013-2022년)**" style="height: 270px;"}
```{python}
#| output: false
import matplotlib.pyplot as plt
import seaborn as sns
...
# 데이터 불러오기
house_data2 = pd.read_csv('data/house_loc.csv', index_col=0)
house2 = house_data2.copy()
house2 = house.loc[ :, ['Utilities','Bsmt_Full_Bath','Full_Bath', 'Bsmt_Half_Bath' ,'Half_Bath', 'Garage_Cars', 'Fireplaces', 'Heating_QC','Central_Air', 'Wood_Deck_SF'] ]
# house는 2930개 열이 있음
```
...
from IPython.display import Image, display
# 텍스트 문구 표시
display(Markdown("**6. 강아지(행복이)가 뛰어놀 수 있는 나무로 된 데크 필수**"))
...
# 지도의 중심을 설정합니다.
map_center = [42.034482, -93.642897]

# 현실 지도 스타일을 적용하여 지도를 생성합니다 (OpenStreetMap 사용).
mymap = folium.Map(location=map_center, zoom_start=15, tiles='OpenStreetMap')

# 로컬 이미지를 Base64로 인코딩합니다.
image_path = "final_house.png"  # PNG 이미지 파일 경로
with open(image_path, "rb") as image_file:
    encoded_image = base64.b64encode(image_file.read()).decode('utf-8')

# 주어진 데이터를 사용하여 데이터 프레임 생성
data = {
    '항목': [
        'Utilities', 'FullBath', 'HalfBath', 'Garage_Cars', 'Fireplaces',
        'Heating_QC', 'Central_Air', 'Wood_Deck_SF', 'Lot_Area', 
        'House_Style', 'Year_Built', 'Year_Remod_Add', 
        'TotRms_AbvGrd', 'Overall_Cond', 'Year_Sold', 
        'Sale_Price', 'Adj_Sale_Price'
    ],
    '세부 사항': [
        'AllPub', '1', '1', '2', '2',
        'Excellent', 'Yes', '216 sqft', '14585',
        'One_Story', '1960', '1987',
        '7', 'Above_Average', '2007',
        '181900', '258661.8'
    ]
}
df = pd.DataFrame(data)

# 데이터 프레임을 HTML로 변환하면서 중앙 정렬을 위한 스타일 적용
table_html = df.to_html(index=False, classes='table table-striped', border=0, justify='center')

# 중앙 정렬을 위한 추가 스타일을 적용
table_html = table_html.replace(
    '<table border="0" class="dataframe table table-striped">',
    '<table border="0" class="dataframe table table-striped" style="text-align: center;">'
)

# 팝업에 이미지를 추가하고 설명과 표를 덧붙입니다.
html = f'''
    <div>
        <img src="data:image/png;base64,{encoded_image}" width="300" height="200"><br>
        <b>남Q의 최종선택 House:</b><br>
        선생님! 제 형편에 이렇게 좋은 집을 살 수 있을지 몰랐는데,, 저는 선생님 능력을 의심하지 않았어요! 정말 감사합니다!!람쥐~~<br><br>
        <b>세부 정보:</b><br>
        {table_html}
    </div>
'''
...
# 마커를 지도에 추가하면서 툴팁을 설정합니다.
_ = folium.Marker(
    location=[42.04074, -93.624484],
    popup=popup,
    tooltip="남Q의 최종선택 House",
    icon=icon
).add_to(mymap)

# Jupyter Notebook에서 직접 지도를 표시합니다.
display(HTML(mymap._repr_html_()))
